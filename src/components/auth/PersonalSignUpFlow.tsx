  ...
  return (
    <div className="relative w-full max-w-md mx-auto z-20">
      ...
      >
        ...
        {/* Step 1: Your Details - Header removed */}
        {currentStep === 1 && (
          <motion.div
        ...
        {/* Step 2: Verify Your Number - Header removed */}
        {currentStep === 2 && (
          <motion.div
            initial={{ opacity: 0, x: 10 }}
            animate={{ opacity: 1, x: 0 }}
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-3.5">
              ...
              <div className="flex justify-between gap-1.5" onPaste={handleOtpPaste}>
                {otpDigits.map((digit, index) => (
                  <input
                    ...
                    className="w-9 h-11 text-center text-sm font-bold bg-neutral-100 hover:bg-neutral-200/60 rounded-xl text-black focus:outline-none focus:bg-neutral-200/90 transition-all"
                  />
                ))}
              </div>
              ...
              <div className="flex items-center justify-between text-xs text-neutral-500">
                ...
                <button
                  ...
                  className="hover:text-black flex items-center gap-1 text-[10px]"
                >
        ...
        {/* Step 3: Build Your Profile - Header removed */}
        {currentStep === 3 && (
          <motion.div
            initial={{ opacity: 0, x: 10 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -10 }}
          >
            <form onSubmit={handleCreateMyWat} className="space-y-2.5">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/45 rounded-xl flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <span className="text-[7px] text-white font-medium">Change</span>
                    </div>
                  </div>

                  {/* Preset Avatars Scroll */}
                  <div className="flex-1 overflow-x-auto pb-0.5 flex gap-1.5 items-center">
                    {PRESET_AVATARS.slice(0, 5).map((pic, idx) => (
                      <button
                        ...
                        className={`w-7 h-7 rounded-lg overflow-hidden shrink-0 transition-transform ${
                          avatar === pic ? 'scale-105 shadow-sm opacity-100' : 'opacity-50 hover:opacity-100'
                        }`}
                      >
