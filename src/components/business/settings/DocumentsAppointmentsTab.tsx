    ...
    <div className="space-y-8 animate-fade-in text-neutral-900">
      ...
      <section className="bg-white rounded-3xl p-6 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3">
          <div>
            <h3 className="text-base font-bold text-neutral-900">Appointments & Consultations</h3>
        ...
        {/* Add Service Form */}
        {isAddingService && (
          <form onSubmit={handleAddService} className="p-4 rounded-2xl bg-white shadow-sm space-y-3">
            <h4 className="text-xs font-bold text-neutral-900 uppercase tracking-wider">Create Bookable Service</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <label className="text-[11px] text-neutral-600 font-bold">Service Title</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[11px] text-neutral-600 font-bold">Duration (Minutes)</label>
                <input
                  type="number"
                  value={newDuration}
              ...
              <div>
                <label className="text-[11px] text-neutral-600 font-bold">Price ($)</label>
                <input
                  type="number"
                  value={newPrice}
              ...
              <div>
                <label className="text-[11px] text-neutral-600 font-bold">Assigned Stylist / Staff</label>
                <input
                  type="text"
                  value={newStaff}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {appointments.services.map((svc) => (
            <div
              key={svc.id}
              className="p-4 rounded-2xl bg-white shadow-sm flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-neutral-900">{svc.title}</span>
                  <span className="text-xs font-bold font-mono text-neutral-900">
                    {svc.price === 0 ? 'FREE' : `$${svc.price}`}
                  </span>
                </div>
                <p className="text-xs text-neutral-600 mt-1 leading-relaxed">{svc.description}</p>
                <div className="flex items-center gap-3 text-xs text-neutral-500 font-mono mt-3">
                  <span className="flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-neutral-400" /> {svc.durationMinutes} mins
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1">
                    <User className="w-3.5 h-3.5 text-neutral-400" /> {svc.assignedStaff}
                  </span>
                </div>
              </div>

              <div className="mt-3 pt-2.5 flex items-center justify-between">
                <span className="text-[10px] text-emerald-700 font-semibold">In-Chat Booking Link Enabled</span>
                <button
                  type="button"
      ...
      {/* Business Documents & Invoicing Templates */}
      <section className="bg-white rounded-3xl p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3">
          <h3 className="text-base font-bold text-neutral-900">Business Documents & Legal Templates</h3>
          <span className="text-xs font-mono text-neutral-600 font-semibold">ISO-AFRICRAFT COMPLIANT</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          {documents.templates.map((doc) => (
            ...
            >
              <div>
                <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase bg-white text-neutral-800 shadow-xs">
                  {doc.type}
                </span>
                <div className="text-xs font-bold text-neutral-900 mt-2">{doc.title}</div>
              ...
              <div className="mt-3 pt-2 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-neutral-900 hover:underline"
                >
                  Preview
                </button>
              </div>
